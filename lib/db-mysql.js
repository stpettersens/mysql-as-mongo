...
var config_file = 'db.mysql.cson';

var data = fs.readFileSync(config_file).toString();
var config = CSON.parse(data);

var schema = config.db.schema;
...
module.exports = {
  ...
  find: function(name, query, limit, callback) {
    ...
    if(typeof(value[0]) == 'string') sql = sql.replace('%d', '"%s"');
  ...
  insertOne: function(name, item, callback) {
    ...
    for(var i = 0; i < a_cols.length; i++) {
        if(typeof(a_items[i]) == 'string') {
          columns += sprintf('%s VARCHAR(50) NOT NULL', a_cols[i]);
          if(i != a_cols.length - 1) columns += ', ';
        }
        else if(typeof(a_items[i]) == 'number') {
          columns += sprintf('%s NUMERIC(15, 2) NOT NULL', a_cols[i]);
          if(i != a_cols.length - 1) columns += ', ';
        }
        else if(typeof(a_items[i]) == 'object') {
          columns += sprintf('%s TIMESTAMP NOT NULL', a_cols[i]);
          if(i != a_cols.length - 1) columns += ', ';
        }
    }
    ...
    connection.query(sprintf('CREATE TABLE IF NOT EXISTS %s.%s (_id VARCHAR(30) NOT NULL PRIMARY KEY, %s);'
    + ' INSERT INTO %s.%s VALUES (\'%s\', %s)',
    schema, name, columns, schema, name, new ObjectID().toString(), inserts));
    callback(null, true);
  },
